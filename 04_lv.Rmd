...
tags$div(style="width:50%; margin-left:auto;  margin-right:auto; font-size:50%",
         DiagrammeR::grViz('scripts/pcload.gv', width='100%', height='400px')
)
```
...
```{r plotMultiFactor, echo=F, eval=T, cache=FALSE}
tags$div(style="width:100%; margin:auto auto;",
DiagrammeR::grViz('scripts/multifactor.gv', width='100%')
)
```
...
```{r secondorder, echo=F, cache=FALSE}
tags$div(style="width:75%; margin:0 auto; text-align='center'",
DiagrammeR::grViz('scripts/second_order.gv', width='100%', height='45%')
)
```
...
```{r bifactor, echo=F, dev.args=list(bg = 'transparent')}
tags$div(style="width:75%; margin:0 auto; text-align='center'",
grViz('scripts/bifactor.gv', width='100%', height='45%')
)
```
...
```{r bifactor_lavaan, eval=F, echo=FALSE}
# won't actually converge with this data
HS.model3 = ' 
visual  =~ x1 + x2 + x3 
textual =~ x4 + x5 + x6
speed   =~ x7 + x8 + x9 
...
# textual ~~ 0*speed
'
fit3 = cfa(HS.model3, data = HolzingerSwineford1939, std.lv = TRUE, information="observed")
summary(fit3)
```
