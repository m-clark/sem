...
ibp = function(alpha, N){
  ...
  for(i in 2:N){
    ...
    last_previously_sampled_dish = which.max(apply(assignments, 2, function(x) any(x>0)))
    dishes_from_previously_sampled = matrix(NA, nrow=1, ncol=last_previously_sampled_dish)
    
    for(k in 1:last_previously_sampled_dish){
      m_k = sum(assignments[1:(i-1),k])                      # fixed, before it was 1:i-1, which would attempt to index 0, though had no effect
      prob = m_k / i
...
res = ibp(1, 100)
d3heatmap::d3heatmap(res, Rowv=F, Colv=F)
