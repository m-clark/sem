...
- Cost: very expensive up front, but notably less so to renew
...
- User manual: None of the output, which for some models is notably complicated, is explained, and the simulated data examples are not very useful and are overly optimistic relative to what researchers will actually face. Too much space is devoted to identical models with only minor differences.
- Closed source: not only that, it's practically one programmer doing all the work.

#### Other

It's not clear to me where the future of Mplus lies.  Bengt Muthén, the driving force behind Mplus has been emeritus faculty for some time now, and version 7 has been out without a full release longer than any other version of Mplus. 'Minor' corrections are typically taking several months.  
...
#### Cons
I would say learning curve, but no one know SEM syntax until they do it, and it will always be different from the syntax typically employed by a statistical program.  If your data is already processed and ready to go, it is no more difficult to run an SEM in R than Stata, and it will be *easier* than Mplus, Amos, etc.
...
Lisrel, EQS etc. are no longer viable these days.  SAS has some functionality but it's rarely used relative to the other options.  SPSS Amos... if you have access to Amos you almost definitely have access to something better and just as easy to use.

#### Other Modeling Tools

Within R one can do mixture/latent class models, growth mixture models, multilevel models, etc.  There is a package that provides functionality for every model depicted in the Mplus manual, and they will almost always provide more tools and output for understanding and exploring those models.  OpenMx, while relatively very difficult compared to other packages, can be used to do any SEM one can think of.
