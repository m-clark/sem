...
# summary(sem_rasch) # not shown as is almost identical to previous
```
...
data.frame(ltm=coef(rasch_mod_par2), 
           ...
           discrm=parameterEstimates(sem_rasch)[23,'est']) %>% 
  round(2) %>% 
  DT::datatable(options=list(dom='t'))
```
...
The only change with the SEM approach is that we allow all the loadings to be estimated, much as we would in a typical SEM setting.
...
#                               select(est))
# scores_ = data.frame(ltm = ltm::factor.scores.ltm(ltm_2pm_par1, resp.patterns=Abortion, method='EAP')$score.dat$z1,
#                      lavaan = lavPredict(sem_2pm))
# 
# difficulties
# discrimination
# head(scores_, 20)
