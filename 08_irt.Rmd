...
```{r 1pm_ICC, echo=F, dev='svglite'}
plotdat = plot(rasch_mod_par2, type='ICC', zrange=c(-2,2), lwd=2, legend=T,
...
In this case we can see that three of the items essentially behave identically, and in general distinguish (slightly less than average) individuals well. The first item would however would take more 'ability' before endorsement, i.e. it is more 'difficult' in test taking terms, but even then it is not too different from the others.  We can now start to think of the latent trait as representing a pro-choice stance, where at the average score the person would likely be endorsing all but the first item.

Another way to look at this is in terms of <span class="emph">item information</span>.  The way one can interpret this is that it tells us which individuals, in terms of the latent trait, are distinguished best by the items.  In this case they more or less distinguish individuals slightly below average for items 2-4, and slightly above for item 1.

```{r 1pm_IIC, echo=F, dev='svglite'}
plotdat = plot(rasch_mod_par2, type='IIC', zrange=c(-2,2), plot=F)
iccdat = gather(data.frame(plotdat), key=Item, value=Information, -z)
ggplot(aes(x=z, y=Information), data=iccdat) +
  geom_line(aes(group=Item, color=Item), lwd=1) +
  xlab(expression('Individual Latent Score')) +
  lazerhawk::theme_trueMinimal()
```
...
```{r 1pm_traitscores, echo=F, dev='svglite'}
...
qplot(rep(factor.scores(rasch_mod_par1)[[1]][['z1']], factor.scores(rasch_mod_par1)[[1]][['Obs']]),
  ...
  geom_point(aes(x=coef,y=0, color=Item), size=3, data.frame(coef=coef(rasch_mod_par2)[,1], Item=factor(paste0('Item.', 1:4)))) +
  lazerhawk::theme_trueMinimal()
```
...
'
sem_rasch <- cfa(sem_rasch_model, data=Abortion_asfactor, std.lv=T)
summary(sem_rasch)
...
The 1PM suggests items only differ by difficulty.  In the SEM approach, this led to the factor loadings being constrained to be equal, which in SEM is probably not a likely scenario. The two parameter IRT model (2PM) allows the discrimination parameter to vary by item. We noted the model before, where $\alpha$, the discrimination paramter was constant, so nothing else has changed besides that aspect.

$$prob(y_{ij}=1|\theta, \delta, \alpha) = \frac{1}{1+e^{\alpha_j(\delta_j-\theta_i)}}$$


Let's see it how this turns out for the abortion data.

```{r ltm2p, echo=1:2}
ltm_2pm_par1 = ltm(Abortion ~ z1, IRT.param=F)
ltm_2pm_par2 = ltm(Abortion ~ z1, IRT.param=T)
# coef(ltm_2pm)
...
We start to see a lack of parallelism in the item characteristic curves.

```{r 2pm_ICC, echo=F, dev='svglite', height=3}
plotdat = plot(ltm_2pm_par2, type='ICC', zrange=c(-2,2), lwd=2, legend=T,
     col=RColorBrewer::brewer.pal(4, 'Set2'), bty='n', plot=F)
# abline(h=.5, v=0, col=scales::alpha('black',.2))
...
plotdat = plot(ltm_2pm_par2, type='IIC', zrange=c(-2,2), plot=F)
iccdat = gather(data.frame(plotdat), key=Item, value=Information, -z)
p2 = ggplot(aes(x=z, y=Information), data=iccdat) +
  geom_vline(xintercept=0, color='black', alpha=.2) +
  geom_line(aes(group=Item, color=Item), lwd=1) +
  xlab('Individual Latent Score') +
  lazerhawk::theme_trueMinimal() +
...
Above, we see that Item 3, 'The woman is not married and does not wish to marry the man.' has little discriminatory ability for anyone above average on the latent trait, as all above average would typically have endorsed that item.  In the testing interpretation, it is a relatively 'easy' item, though not too different from items, 2 and 4. However, as it is relatively narrower, it is less able to distinguish individuals on the latent trait relative to other items. Item 1 on the other hand, 'The woman decides on her own that she does not.', doesn't discriminate well those who are low-scoring on the latent trait, but does for those on the high end.  In the testing interpretation, this would be a relatively difficult item.
...
The only change with the SEM approach is that we allow all the loadings to be estimated, much as we would with typical SEM models. The following shows the necessary model syntax.
...
data.frame(ltm_diff = coef(ltm_2pm_par1)[,'(Intercept)'],
           lav_diff = lavpars[23:26, 'est']*1.7,
           ltm_disc = coef(ltm_2pm_par1)[,'z1'],
...
data.frame(ltm_diff = coef(ltm_2pm_par1)[,'(Intercept)']/1.7,
           lav_diff = lavpars[23:26, 'est'],
           ltm_disc = coef(ltm_2pm_par1)[,'z1']/1.7,
...
sem_2pm <- cfa(sem_2pm, data=Abortion_asfactor, std.lv=T, link='probit')
summary(sem_2pm)
...
$$prob(y_{ij}=1|\theta, \delta, \alpha) = \gamma_j + \frac{1-\gamma_j}{1+e^{\alpha_j(\delta_j-\theta_i)}}$$

The model has the effect of including a lower bound on responding to the 2PM, and could vary by item as well.  While it probably isn't as applicable to the Abortion data, one can think of it as an offset or propensity/bias to endorse, and such a model in general might be suited to more imbalanced data response.

```{r 3pm_ICC, echo=FALSE}
load('data/3pm.RData')
...
Here we can see item 2. 'The couple agree that they do not wish to have a child.', does have an asymptote slightly above 0, where the others are estimated to be zero. 

### 4 Parameter Model

As one might have already been thinking, just as we could have a lower bound, we can also add an upper bound to the probability of endorsement.  In the testing scenario, this would regard very difficult items that even those high on the latent trait might not have a high probability of being correct.

$$prob(y_{ij}=1|\theta, \delta, \alpha) = \gamma_j + \frac{\zeta_j -\gamma_j}{1+e^{\alpha_j(\delta_j-\theta_i)}}$$

See the <span class="pack">sirt</span> package <span class="func">rasch.mml2</span> for a means to estimate such models.

## Other IRT Models

### Additional covariates

If you think back to the Rasch model as a mixed model, it is very easy to add person level characteristics to the model.  One would think, and especially in the case of non-testing situations, that any number of demographic contexts might influence item endorsement.  As such, one might consider them when doing IRT as well.
...
## R Packages Used
https://blogs.baylor.edu/rlatentvariable/sample-page/r-syntax/#Item_response_theory_models
