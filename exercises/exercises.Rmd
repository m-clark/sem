...
Judging from the following correlation matrix (blue= positive correlation, red=negative, bold=stronger, faded=weaker), can you already see some structure appearing? How many factors do you see? How are they correlated with one another?

```{r, echo=FALSE, eval=TRUE}
load('data/bfi.R')
bfi_trim %>% cor(use='pair') %>% lazerhawk::corrheat(psychOptions = list(fm='ml', rotate='promax'))
```


1. Load the data, `load('data/bfi.RData')`, and run a factor analysis for five factors and interpret the results. The following provides a hint. You can use fa.diagram for a quick and dirty graphical depiction.
...
Compare models for 1:7 in one line of code. Run the following.
