...
output:
  html_document:
    ...
    toc_float: TRUE
    code_folding: hide
  # html_notebook:  # there is nothing to be gained by notebook aside from caching issues, no svg, broken css etc.
  #   # css: ../sem.css # using css will mess up line breaks in code
  #   theme: sandstone
  #   font-family: Roboto
  #   highlight: zenburn
  #   toc: TRUE
...
First get the data in, then set up your model code.  The following provides a hint. Note that in this model you have *two* outcomes/endogenous varialabes, so write the model code pertaining to each.
...
If time allows do both, otherwise, pick one of the following.
...
suppressMessages(lazerhawk::corrheat(cov2cor(marshCov), width='100%') %>% 
