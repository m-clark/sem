...
Consider the following code regarding a 3 variable mediation problem, and draw the corresponding path model.  Which is the covariate, mediator, and outcome? Using the 'product-of-paths' approach, what value should the indirect effect be in the population?  
...
Rerun with the model to explicitly test the indirect effects using the product-of-paths approach and labeling the paths in the lavaan model.  Click to see how. 
...
The following code creates an adjacency matrix, the corresponding graph (for 10 nodes), and a couple measures for nodes.  Play around with it to see what you come up with.  For example, if you reverse the probabilities for the 0s and 1s, how will the graph change?
